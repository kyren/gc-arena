...
///    recorded. Calling `Collect::trace` will usually mark other `Gc`s as reachable and queue them
///    for tracing if they have not already been, so this step may also transitively perform other
///    work, but each step is only performed exactly once for each individual remembered value.
...
/// reachable (`mark_factor * alloc_size` work is recorded), then during sweeping it will be
/// *dropped* (`drop_factor * alloc_size` work is recorded), and then *kept* (`keep_factor *
