...
// 'static and thus also possibly Send. This must be correct though, because in order for a type to
// be 'gc without unsafety, any 'gc types it contains must have come from `gc-arena` and actually be
// "'static and in an arena".
