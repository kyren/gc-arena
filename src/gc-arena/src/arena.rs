...
//
// The 'gc lifetime generated by Arena callbacks is, effectively "'static and in an arena". The
// callback is required to work for any given 'gc, and the only thing that provides types with this
// lifetime in the callback are gc-arena provided methods. Since the callback is required to work
// for *any* 'gc, so it must also work for 'static, so it cannot be unsound for 'gc to *actually*
