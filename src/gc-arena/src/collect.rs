...
/// A trait for garbage collected objects that can be placed into [`Gc`](crate::Gc) pointers. This trait
/// is unsafe, because `Gc` pointers inside an Arena are assumed never to be dangling, and in order to
